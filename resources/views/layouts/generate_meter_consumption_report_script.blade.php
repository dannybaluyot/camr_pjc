...
<script type="text/javascript">
	...
	$("#generate_raw_report").click(function(event){
			...
			let end_time 			= $("input[name=end_time]").val();
			  ...
			  $.ajax({
				url: "/generate_consumption_report/"+interval_type,
				type:"POST",
				  ...
				  if(response['data']!='') {
					...
					$('#end_dateError').text('');	
						...
						for(var i=0; i<len; i++){
							...
							total_current_consumption += _kwh_total;
						
							var data_count = i+1;
							addData(datetime,kwh_total,data_count);
							
						}		
							
							LoadConsumptionData.clear().draw();
							LoadConsumptionData.rows.add(response.data).draw();	
							...
							$('#total_current_consumption').text(total_current_consumption.toFixed(3));
							
							$('#total_KWh_txt').text(total_current_consumption.toFixed(3));
		...
		/*Load to Dtatables*/	
		let LoadConsumptionData = $('#meter_consumption_html_table').DataTable( {
				"language": {
				...
				/*3*/	{data: 'min_wh_total',  orderable: false, render: $.fn.dataTable.render.number( ',', '.', 3, '' ) },
				/*4*/	{data: 'max_datetime', className: "text-center", orderable: false },		
				/*5*/	{data: 'max_wh_total', orderable: false, render: $.fn.dataTable.render.number( ',', '.', 3, '' ) },	
				/*6*/	{data: 'multiplier', className: "text-right", orderable: false },
				/*7*/	{data: 'kwh_total', className: "text-right", orderable: false, render: $.fn.dataTable.render.number( ',', '.', 3, '' ) }
				],

		} );				
	
	autoAdjustColumns(LoadConsumptionData);
	...
	function download_report(){
		...
		var url = "{{URL::to('/download_consumption_report/')}}?" + $.param(query)
		window.open(url);
